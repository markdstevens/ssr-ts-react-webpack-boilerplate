...
/**
 * @description A light wrapper around axios.get that adds logging for errors and request duration
 * 
 * @param {string} url The URL used to retrieve data from axios.get
 * 
 * @returns {Promise<T>} A promise containing the response from axios
 */
export async function axiosWrapper<T>(uri: string): Promise<T> {
  try {
    logger.event(Event.AXIOS_REQUEST, loggerUtils.convertToLogString(url.parse(uri)));
